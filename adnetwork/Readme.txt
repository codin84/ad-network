How to build and use AdNetwork application:
    - application is made using Spring boot and maven, so just:
        ...
        - in folder containing downloaded code run command:
            - mvn clean install -DskipTests
            - mvnw spring-boot:run

After necessary dependencies are downloaded and application is started,
...
Web app is used so that it demonstrates, that we cannot import adNetworks from sites that are not configured in our application (url should be defined, date format,...).
Configuration for each network is found in a table ad_network_source. Ad_network_source is pre-filled at startup with 2 providers from data.sql file.
