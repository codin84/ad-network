...
public class RevenueValueParser
{
  public static RevenueValue parse(@Nonnull final String value, @Nonnull final String revenueHeader)
  {
    ...
    {
      if (currencyFromValue == null) throw new IllegalArgumentException("No currency found!");

      return new RevenueValue(value.replaceAll(currencyFromValue.getSymbol(), ""), currencyFromValue);
    }

    final Currency currencyFromHeader = Arrays.stream(Currency.values()).filter(currency -> revenueHeader.toUpperCase().contains(currency.name())).findFirst().orElseThrow(() -> new IllegalArgumentException("Currency not found for header (" + revenueHeader + ")"));
  ...
  {
    Assert.hasText(value, "Value must not be empty!");
    return Arrays.stream(Currency.values()).filter(currency -> value.startsWith(currency.getSymbol()) || value.endsWith(currency.getSymbol())).findFirst().orElse(null);
  }
